...
test('GET de dados de trabalhadores exige autenticação', async ({ request }) => {
  const response = await request.get(EMPLOYEES_URL);
  expect(response.status()).toBe(401);
});

test('PATCH de registro exige autenticação ou autorização', async ({ request }) => {
...
test('preflight expõe CORS amplo sem credenciais', async ({ request }) => {
