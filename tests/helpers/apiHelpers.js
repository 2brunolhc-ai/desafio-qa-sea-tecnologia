...
export const API_BASE_URL = (process.env.API_BASE_URL || 'https://analista-teste.seatecnologia.com.br').replace(
  /\/$/,
  '',
);
...
// Evita incluir documentos pessoais em diagnósticos de parse.
...
export async function createEmployee(request, data) {
  ...
  if (rawBody.trim()) {
    try {
      ...
      parseDiagnostic = [parseDiagnostic, `Corpo não pôde ser desserializado como JSON: ${preview}`]
        .filter(Boolean)
        .join(' ');
    }
  }
...
export async function cleanupEmployee(request, record) {
  ...
  const markers = [record.testMarker, record.state?.testMarker, employee.name, employee.testMarker, employee.rg];
  const isOwned = markers.some((value) => typeof value === 'string' && value.startsWith(TEST_DATA_PREFIX));

  // Nunca tenta apagar um registro que não pertença à automação.
  if (!isOwned) {
    throw new Error(`Limpeza recusada para registro sem marcador ${TEST_DATA_PREFIX}.`);
  }

  const response = await request.delete(`${EMPLOYEES_URL}/${encodeURIComponent(String(record.id))}`);
