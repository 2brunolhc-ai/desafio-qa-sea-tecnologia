  ...
  const contrasts = await page.evaluate(() => {
    ...
    const candidates = [
      ...
      [...document.querySelectorAll('button')].find((element) =>
        element.textContent?.includes('Adicionar Funcionário'),
      ),
      [...document.querySelectorAll('button')].find((element) => element.textContent?.includes('Ver apenas ativos')),
      [...document.querySelectorAll('button')].find((element) => element.textContent?.includes('Próximo passo')),
    ].filter(Boolean);
...
test('[BUG-023] interface rejeita CPF com letras antes do POST', async ({ page, request }) => {
  ...
  try {
    await openEmployeeForm(page);
    await fillEmployeeForm(page, employee);
    const response = await optionalPostAfter(page, () => page.getByRole('button', { name: 'Salvar' }).click());
    if (response?.status() === 201) created = await response.json();
...
test('[CONTROLE-FILTRO] filtro de ativos oculta inativo e Limpar filtros restaura ambos', async ({ page }) => {
  ...
  const activeName = page.getByText(active.state.employee.name, {
    exact: true,
  });
  const inactiveName = page.getByText(inactive.state.employee.name, {
    exact: true,
  });
